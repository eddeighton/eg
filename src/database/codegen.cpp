...
namespace eg
{
    ...
    void generateIncludeGuard( std::ostream& os, abstract::Root* pRoot )
    {
    ...
    void generateInterfaceIncludes( std::ostream& os, abstract::Root* pRoot, 
    ...
    void generateForwardDeclarations( std::ostream& os, abstract::Root* pRoot )
    {
        abstract::Element::Collector< input::Action > actions;
        pRoot->visit( actions );
        abstract::Element::Collector< input::Dimension > dimensions;
        pRoot->visit( dimensions );
        
        std::map< std::string, int > identifiers;
        for( const input::Action* pAction : actions() )
            identifiers.insert( std::make_pair( pAction->getIdentifier(), pAction->getIndex() ) );
        for( const input::Dimension* pDimension : dimensions() )
            identifiers.insert( std::make_pair( pDimension->getIdentifier(), pDimension->getIndex() ) );
        
        os << "\n//Forward declarations\n";
        for( std::map< std::string, int >::const_iterator 
            i = identifiers.begin();
            i != identifiers.end(); i = identifiers.upper_bound( i->first ) )
        {
            if( input::Root::RootTypeName != i->first )
                os << "struct [[clang::eg_type( -" << i->second << " )]]" << i->first << ";\n";
        }
    }
    ...
    void generateInterface( std::ostream& os, abstract::Root* pRoot )
    {
        ...
        generateForwardDeclarations( os, pRoot );
    ...
    void printTypePathType( std::ostream& os, const IndexedObject::Array& objects, const InvocationSolution& invocation )
    {
        ...
        for( EGTypeID id : typePath )
        {
            ...
            {
                const input::Element* pElement = dynamic_cast< const input::Element* >( objects[ -id ] );
                ASSERT( pElement );
                switch( pElement->getType() )
                {
                    case eInputDimension : os << dynamic_cast< const input::Dimension* >(  pElement )->getIdentifier(); break;
                    case eInputAction    : os << dynamic_cast< const input::Action* >(     pElement )->getIdentifier(); break;
                    case eInputRoot      : 
                    case eInputOpaque    :
                    case eInputInclude   :
                    default:
