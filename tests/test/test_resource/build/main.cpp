...
static std::unique_ptr< std::array< b_root_PrintRes, 1 > > g_root_PrintRes_allocation;
b_root_PrintRes* g_root_PrintRes = nullptr;
void allocate_buffers()
{
    ...
    for( eg::Instance i = 0U; i != 1; ++i )
    {
        ...
        g_root[ i ].g_root_reference = __eg_root< void >( eg::reference { i, 19, 0 } );
        g_root[ i ].g_root_ring_index = i;
        g_root[ i ].g_rootPrintRes_ring_iter = 0UL;
    }
    g_root_PrintRes_allocation = std::make_unique< std::array< b_root_PrintRes, 1 > >();
    g_root_PrintRes = g_root_PrintRes_allocation->data();
    for( eg::Instance i = 0U; i != 1; ++i )
    {
        g_root_PrintRes[ i ].g_root_PrintRes_cycle = eg::INVALID_TIMESTAMP;
        g_root_PrintRes[ i ].g_root_PrintRes_state = ::eg::action_stopped;
        g_root_PrintRes[ i ].g_root_PrintRes_reference = __eg_root< void >::__eg_PrintRes< void >( eg::reference { i, 26, 0 } );
        g_root_PrintRes[ i ].g_root_PrintRes_ring_index = i;
        g_root_PrintRes[ i ].g_root_PrintRes_ring = i;
    }
}
