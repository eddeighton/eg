...
//Buffer: b_root stride: 48 size: 1
struct b_root
{
...
//Buffer: b_root_Test stride: 44 size: 1
struct b_root_Test
{
...
static std::array< b_root_Test, 1 > g_root_Test;
...
void allocate_buffers()
{
    for( EGInstance i = 0U; i != 1; ++i )
    {
        ...
        g_root[ i ].g_rootTest_alloc_iter = 0UL;
    }
    for( EGInstance i = 0U; i != 1; ++i )
    {
        g_root_Test[ i ].g_root_Test_timestamp_runnning = EG_INVALID_TIMESTAMP;
        g_root_Test[ i ].g_root_Test_timestamp_paused = EG_INVALID_TIMESTAMP;
        g_root_Test[ i ].g_root_Test_reference = __interface_root< void >::__interface_Test< void >{ i, 0 };
        g_root_Test[ i ].g_root_Test_timestamp_stopped = 0U;
        g_root_Test[ i ].g_root_Test_alloc_data = i;
    }
}
...
extern __interface_root< void >::__interface_Test< void > root_Test_starter( EGInstance _gid );
extern void root_Test_stopper( EGInstance _gid );
extern bool root_Test_executor();
...
struct BasicHost_EGDependencyProvider : public EGDependencyProvider
{
    ...
    virtual void* getBuffer( const char* pszName )
    {
        ...
        if( 0U == strcmp( pszName, "g_root_Test" ) ) return g_root_Test.data();
        return nullptr;
    }
...
bool executeSchedule()
{
    ...
    bWaited = root_Test_executor() || bWaited;
    return bWaited;
}


std::string strEventLogFolder = "test2_log";
